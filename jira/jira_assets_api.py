    ...
    for workspace_id, obj_map in workspace_map.items():
        ...
        for i in range(0, len(object_ids), _ASSETS_RESOLVE_BATCH):
            ...
            try:
                ...
                resp.raise_for_status()
                data = resp.json()
                # JSM v1 returns {"values": [...]}, older endpoint returns {"objectEntries": [...]}
                entries = data.get("values") or data.get("objectEntries") or []
